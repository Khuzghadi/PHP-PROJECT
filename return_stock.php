...
if(isset($_POST['submit'])){
    ...
    if($product_id <= 0 || $qty <= 0){
        ...
        $stmt = $conn->prepare("INSERT INTO stock_requests (user_id, product_id, quantity, request_type) VALUES (?, ?, ?, 'return')");
        $stmt->bind_param("iii",$uid,$product_id,$qty);
        $stmt->execute();
        $stmt->close();

        $act = $conn->real_escape_string("Requested return: ProductID $product_id (+$qty) by user $uid");
        $conn->query("INSERT INTO activity_log (user_id, action) VALUES ($uid, '$act')");

        header("Location: user_dashboard.php?msg=" . urlencode("Return request submitted"));
        exit;
    }
}
?>
...
<!DOCTYPE html>
<html>
<head><title>Return Stock</title></head>
<body class="bg-light">
...
<form method="POST" class="card p-3 shadow-sm" style="max-width:600px">
  ...
  <button name="submit" class="btn btn-warning">Submit Return Request</button>
</form>
